...
- **`PaperRef`** (`paper.rs`) — a citation key (e.g. `turing1936`) addressing the closed set of papers already declared in the local library. Used by `remove`/`edit`/`fetch` (implemented) and `open` (not yet — see `docs/status.md`).
