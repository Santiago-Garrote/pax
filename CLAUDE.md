...
Intended workflow: `Search → Select → Declare → Fetch → Manage → Reproduce` (see `docs/mvp.md` for the full command surface and design philosophy — `pax add`, `pax fetch`, `pax sync`, `pax check`, `pax export bibtex`, etc. are planned but not yet implemented).

**This repository's primary artifact is `pax-core`, a library — not a CLI tool that happens to have a core module.** The `pax` binary (`src/bin/pax.rs`) is a thin, optional client of that library, gated behind the `cli` Cargo feature; a future `lazypax` TUI (or any other adapter) is meant to be just another client of the same library, per the project's own README. Only `pax init` and `pax search <query>` exist today. Everything else in `docs/mvp.md` is a forward-looking spec, not current behavior — don't assume a command exists just because it's documented there.
...
**Reference types — the load-bearing design decision in this codebase:** `add`/`show` (not yet implemented) and `search` operate on fundamentally different address spaces, so they use different types:
...
## Known gaps (see `docs/mvp.md` for the full target command surface)

`add`, `show`, `remove`, `edit`, `list`, `fetch`, `sync`, `check`, `export bibtex` are all unimplemented — this restructuring exists so each can be added as a small, well-typed addition on top of `Provider`/`CandidateId`/`Library`/`PaperRef`, rather than needing another architectural change first.
